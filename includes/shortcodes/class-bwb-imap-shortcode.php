...
class BWB_Imap_Shortcode {
    ...
    public static function init() {
        add_shortcode( 'foulkeways_map', array( __CLASS__, 'render_map_shortcode' ) );
        add_shortcode( 'foulkeways_interactive_map', array( __CLASS__, 'render_map_shortcode' ) );
    ...
    public static function render_map_shortcode( $atts ) {
        ...
        wp_enqueue_script( 'foulkeways-map-frontend-view' );
        wp_enqueue_style( 'foulkeways-map-styles' );

        return sprintf(
            '<div id="foulkeways-interactive-map-root" class="foulkeways-map-container foulkeways-map-shortcode-container" style="width:100%%; height:%s; position:relative; z-index:1; isolation:isolate;" data-height="%s" data-zoom="16" data-tilt="0" data-map-type="hybrid"></div>',
