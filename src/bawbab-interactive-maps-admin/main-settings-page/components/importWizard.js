...
export const GeoJSONImportWizardModal = ( {
		...
		const inspect = async () => {
			...
			try {
				...
				if ( isMounted && res.success ) {
					...
					keys.forEach( ( k ) => {
						initialCustom[ k ] =
							! allMappedSystemKeys.includes( k );
					} );
					...
					setInspectionError(
						res.message ||
							__(
								'Failed to inspect GeoJSON structure.',
								TEXT_DOMAIN
							)
					);
				}
				...
				if ( isMounted ) {
					setInspectionError(
						__(
							'Network error during file inspection.',
							TEXT_DOMAIN
						)
					);
				}
	...
	const handleImportSubmit = async () => {
		if ( ! enabledStandardFields.fid || ! standardMapping.fid ) {
			alert(
				__(
					'Please map a column to the required Feature ID (fid).',
					TEXT_DOMAIN
				)
			);
			return;
		...
		const reservedSystemColumns =
			Object.values( standardMapping ).filter( Boolean );

		const selectedCustomList = [];
		Object.keys( customSelections ).forEach( ( k ) => {
			if (
				customSelections[ k ] &&
				! reservedSystemColumns.includes( k )
			) {
	...
	const reservedSystemColumns =
		Object.values( standardMapping ).filter( Boolean );
	const dynamicKeys = detectedKeys.filter(
		( k ) => ! reservedSystemColumns.includes( k )
	);
			...
			{ isInspecting ? (
				...
				<div>
					...
					<div style={ { marginBottom: '20px' } }>
						...
						>
							{ standardFields.map( ( field ) => {
								const isChecked =
									enabledStandardFields[ field.key ];
								const isAuto =
									field.supportsAuto &&
									autoComputeCoords[ field.key ];
					...
					<div>
						...
						>
							{ __(
								'2. Dynamic GeoJSON Custom Attributes (Check to Import)',
								TEXT_DOMAIN
							) }
						</h3>
