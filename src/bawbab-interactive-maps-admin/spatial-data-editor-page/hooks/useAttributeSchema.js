...
export const useAttributeSchema = () => {
    ...
    // 2. Add or update a key in the central attribute schema (includes icon support)
    const updateSchemaKey = useCallback(async ({ key, label, type, icon }) => {
        try {
            const response = await fetch('/wp-json/bwb-imaps-federated-api/v1/update-attribute-schema', {
                ...
                body: JSON.stringify({ key, label, type, icon })
            });
