 ...
 */
export const useMapCredentialsCheck = (
	isLoaded,
	googleApiKey,
	googleMapId
) => {
	...
	useEffect( () => {
		...
		if ( isLoaded && ! hasDispatchedNoticeRef.current ) {
			...
			if ( ! googleApiKey || googleApiKey.trim() === '' ) {
				createWarningNotice(
					__(
						'Bawbab Interactive Maps: Please enter a valid Google Maps API Key under the map settings tab to enable public rendering layouts.',
			...
			if ( ! googleMapId || googleMapId.trim() === '' ) {
				createWarningNotice(
					__(
						'Bawbab Interactive Maps: Please enter a valid Google Map ID for your maps to load correctly.',
						'bawbab-interactive-maps'
					),
					{
