...
export const useSpatialDataImporter = () => {
	...
	const importGeoJSON = useCallback(
		async ( {
			...
			console.log(
				'[useSpatialDataImporter] Starting GeoJSON import with payload:',
				{
					layerType,
					fieldMapping,
					importedCustomKeys,
			...
			formData.append(
				...
				JSON.stringify( importedCustomKeys )
			);
			formData.append(
				'overwrite_existing',
			...
			try {
				...
				try {
					...
					console.error(
						'[useSpatialDataImporter] Raw server response was not valid JSON:',
						text
					);
					setMessage( {
						type: 'error',
				...
				console.log(
					'[useSpatialDataImporter] Server import response:',
					result
				);
				...
				console.error(
					'[useSpatialDataImporter] Network error during import:',
					error
				);
				setMessage( {
					type: 'error',
