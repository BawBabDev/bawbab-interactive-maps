...
export const AttributeSchemaManager = ( {
	...
	return (
		<div style={ { padding: '20px 0' } }>
			...
			>
				{ __(
					'Manage custom fields across all map features. Adding a field registers it globally; deleting a field purges it from all units in the database.',
					TEXT_DOMAIN
				) }
			</Text>
					...
					<div className="bwb-select-control-wrapper">
						<SelectControl
							...
							options={ [
								...
								{
									label: __( 'Boolean', TEXT_DOMAIN ),
									value: 'boolean',
								},
								{
									label: __( 'Bathrooms', TEXT_DOMAIN ),
									value: 'bathrooms',
								},
							] }
							onChange={ setNewType }
			...
			{ isLoading ? (
				...
				>
					{ __(
						'No custom attribute keys registered yet. Create one above or import a GeoJSON dataset.',
						TEXT_DOMAIN
					) }
				</p>
			) : (
