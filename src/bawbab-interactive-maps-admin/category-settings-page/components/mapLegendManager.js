...
export const MapLegendManager = ({ 
    ...
    const handleToggleItem = (secId, itemId, checked) => {
        setLegendConfig(prev => ({
            ...
            sections: prev.sections.map(sec => {
                ...
                return {
                    ...sec,
                    items: sec.items.map(item => item.id === itemId ? { ...item, showInLegend: checked } : item)
                };
            })
        }));
    };
    ...
    const handleUnmergeItem = (secId, itemId) => {
        ...
        sec.items = sec.items.filter(i => i.id !== itemId).concat(restoredItems);
        setLegendConfig(prev => ({ ...prev, sections }));
    };
    ...
    return (
        <Panel style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '6px', marginBottom: '25px' }}>
            <PanelBody 
                title={__('3. Map Legend Customizer & Sections', TEXT_DOMAIN)} 
                opened={isOpen}
                onToggle={onToggle}
            >
                ...
                {legendConfig.enabled && (
                    <div>
                        ...
                        {(legendConfig.sections || []).map((section, secIdx) => (
                            <div key={section.id} style={{ marginBottom: '20px', border: '1px solid #e0e0e0', borderRadius: '4px', background: '#fff', overflow: 'hidden' }}>
                                <div style={{ padding: '10px 12px', background: '#f5f5f5', borderBottom: '1px solid #e0e0e0' }}>
                                    <Flex align="center" justify="space-between" gap={3}>
                                        <FlexItem style={{ flex: 1 }}>
                                            <TextControl
                                                value={section.title}
                                                onChange={(val) => handleRenameSection(section.id, val)}
                                        ...
                                        <Button
                                            ...
                                            label={__('Delete Section Header', TEXT_DOMAIN)}
                                        />
                                    </Flex>
                                </div>
                                            ...
                                            >
                                                ...
                                                <div style={{ textAlign: 'center' }}>
                                                    <CheckboxControl
                                                        ...
                                                        onChange={(checked) => handleToggleItem(section.id, item.id, checked)}
                                                        __nextHasNoMarginBottom
                                                    />
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    {item.type === 'merged' && (
                                                        <span style={{ fontSize: '10px', color: '#2271b1', marginLeft: '6px', background: '#f0f6fb', padding: '2px 4px', borderRadius: '3px' }}>
                                                            {sprintf(__('[Merged %d]', TEXT_DOMAIN), item.categories.length)}
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                <Flex align="center" gap={1}>
                                                    {item.categories.map(compositeKey => {
                                                        ...
                                                        return (
                                                            <div 
                                                                key={compositeKey}
                                                                title={`${categoryMap[compositeKey]?.label || compositeKey} (${catColor})`}
                                                                style={{
                                                                    width: '16px',
                                                                    height: '16px',
                                                                    borderRadius: '50%',
                                                ...
                                                <div style={{ textAlign: 'center' }}>
                                                    {item.type === 'merged' && (
                                                        <Button
                                                            ...
                                                            isDestructive
                                                            icon="editor-break"
                                                            onClick={() => handleUnmergeItem(section.id, item.id)}
                        ...
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', paddingTop: '15px', borderTop: '1px solid #eee' }}>
                            <div style={{ flex: 1 }}>
                                <TextControl
                                    label={__('Add Custom Legend Section Header', TEXT_DOMAIN)}
                ...
                >
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <SelectControl
                            ...
                            options={(legendConfig.sections || []).map(s => ({ label: s.title, value: s.id }))}
