...
export const CampusNavigation = ( {
	...
	useEffect( () => {
		...
		const initializeNavigation = async () => {
			console.log(
				'🧭 [CampusNavigation] Initializing routing context panels...'
			);
			...
			if ( graphData ) {
				...
				const doors = findIntersectingDoors(
					destinationFeature,
					graphData.entries
				);
				...
				if ( doors.length > 0 ) {
					...
					console.log(
						'🎯 [CampusNavigation] Initial destination target mapped:',
						doors[ 0 ]
					);
				} else {
					console.warn(
						'⚠️ [CampusNavigation] No door found near selected building envelope bounds.'
					);
				}
			}
		};
	...
	useEffect( () => {
		if ( manualOriginNode ) {
			console.log(
				'📍 [CampusNavigation] Received manual origin point selection:',
				manualOriginNode
			);
	...
	const handleExecuteRouting = async () => {
		if ( ! selectedDoor || graphEntries.length === 0 ) {
			console.warn(
				'⚠️ [CampusNavigation] Execution aborted: target door properties or node entries array are missing.'
			);
			return;
		}
		...
		if ( ! startPoint ) {
			console.log(
				'ℹ️ [CampusNavigation] No pinned origin point set. Resolving fallback gate nodes array...'
			);
			const gateways = graphEntries.filter(
				( e ) =>
			...
			console.log(
				'ℹ️ [CampusNavigation] Final chosen starting point:',
				startPoint
			);
		}
			...
			{ isGraphLoading ? (
				...
				<>
					...
					>
						...
						{ manualOriginNode ? (
							...
							>
								...
								/>
								<span>
									{ manualOriginNode.name ||
										sprintf(
											__(
												'Selected Node ID: %d',
												'bawbab-interactive-maps'
											),
											manualOriginNode.fid
										) }
								</span>
							</div>
						) : (
							...
							>
								{ __(
									'No starting point selected (Defaulting to Main Gate Entrance)',
									'bawbab-interactive-maps'
								) }
							</span>
						) }
						...
						<Button
							...
							className={ `is-secondary ${
								isPickingOrigin ? 'is-pressed' : ''
							}` } // Binds contextual active styling variables dynamically
						...
						>
							<Dashicon
								icon={ isPickingOrigin ? 'stop' : 'location' }
							/>
							{ isPickingOrigin
								? __(
										'Cancel pin assignment...',
										'bawbab-interactive-maps'
								  )
								: __(
										'Pin Origin location from Map',
										'bawbab-interactive-maps'
								  ) }
						</Button>
					</div>
					...
					{ matchingDoors.length > 1 && (
						<SelectControl
							label={ __(
								'Select Destination Door:',
								'bawbab-interactive-maps'
							) }
							value={
								selectedDoor
									? JSON.stringify( selectedDoor )
									: ''
							}
							...
							onChange={ ( val ) =>
								setSelectedDoor( JSON.parse( val ) )
							}
						/>
					) }
					...
					{ matchingDoors.length === 0 && (
						...
						>
							{ __(
								'No door points discovered inside search buffer boundaries.',
								'bawbab-interactive-maps'
							) }
						</div>
					) }
