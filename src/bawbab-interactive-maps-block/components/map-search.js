...
export const MapSearch = ({ spatialFeatures, locations, onSelect, navBackgroundProp, logoProp }) => {
    ...
    const allNamedFeatures = useMemo(() => {
        ...
        spatialFeatures.forEach(f => {
            if (f.properties.name) list.push({ ...f.properties, geometry: f.geometry, type: 'spatial' });
        });
        locations.forEach(loc => {
            if (loc.title) list.push({ ...loc, name: loc.title, type: 'marker' });
        });
        return list;
    ...
    return (
        <div className={`map-theme-${colorTheme}`}>
            <div 
                ...
                style={{ '--nav-bg': navBackground ? `url(${navBackground})` : 'none' }}
            >
                <button className={`burger-btn ${isBurgerOpen ? 'active' : ''}`} onClick={() => setIsBurgerOpen(!isBurgerOpen)}>
                        ...
                        <div className="autocomplete-results">
                            {filteredResults.map((item, idx) => (
                                <div key={idx} className="result-row" onClick={() => onSelect(item)}>
                                    <span className="result-name">{item.name} {item.code ? `(${item.code})` : ''}</span>
                                    <span className="result-category">{item.category?.replace('_', ' ')}</span>
                                </div>
                            ))}
                        </div>
                    )}
