...
export const ZoomHandler = ( {
	...
	useEffect( () => {
		...
		const currentLocationKey = `${
			selectedLocation.layer_type ||
			selectedLocation.properties?.layer_type ||
			'loc'
		}-${
			selectedLocation.fid ||
			selectedLocation.properties?.fid ||
		...
		if ( selectedLocation.lat && selectedLocation.lng ) {
			...
			if ( Array.isArray( ring ) && ring.length > 0 ) {
				const lats = ring.map( ( c ) =>
					Array.isArray( c ) ? c[ 1 ] : c.lat
				);
				const lngs = ring.map( ( c ) =>
					Array.isArray( c ) ? c[ 0 ] : c.lng
				);
		...
		const panToTargetWithOffset = () => {
			...
			const sidebarEl =
				containerEl?.querySelector( sidebarSelector ) ||
				document.querySelector( sidebarSelector );
			const sidebarWidth =
				sidebarEl && isDrawerOpen
			...
			if ( ! isMobile && ! editMode && sidebarWidth > 0 ) {
				...
				if ( projection ) {
					...
					const finalTarget =
						projection.fromPointToLatLng( worldPoint );
