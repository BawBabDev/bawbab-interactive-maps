            ...
            {extraAttrKeys.length > 0 && (
                <div className="generic-specs-badges" style={{ marginTop: '15px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {extraAttrKeys.map((key) => {
                        ...
                        return (
                            <div key={key} style={{ padding: '6px 12px', background: '#f0f4f8', borderRadius: '12px', border: '1px solid #d0d7de', fontSize: '12px' }}>
                                <strong>{formatFieldLabel(key)}:</strong> {displayVal}
                            </div>
                        );
                    })}
                </div>
