...
const MapLabelItem = memo(
	( {
		...
		if ( ! visibleLayers[ layer_type ] || show_label === false )
			return null;
		...
		const renderElevatorIcon =
			layer_type === 'paths' && isElevatorRoom( name );
		...
		return (
			<>
				{ visibleViewportFeatures.map( ( f, i ) => (
					<MapLabelItem
						key={ `label-${ f.properties?.layer_type }-${
							f.properties?.fid || i
						}` }
						feature={ f }
