...
import { PanelBody, RangeControl, Placeholder, Spinner, TextControl } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }) {
    ...
    if (isLoading) {
        return (
            <div {...blockProps}>
                <Placeholder label={__('Loading Map Data...', 'bawbab-interactive-maps')}>
                    <Spinner />
                </Placeholder>
            </div>
        );
	...
	return (
		<div { ...blockProps }>
			{ /* Sidebar Settings */ }
			<InspectorControls>
				<PanelBody title={__('Map display Settings', 'bawbab-interactive-maps')}>
					<RangeControl
						label="Default Zoom Level"
						value={ attributes.zoom }
						onChange={ ( val ) => setAttributes( { zoom: Math.max(MIN_MAP_ZOOM, val) } ) }
						min={ MIN_MAP_ZOOM }
						max={ MAX_MAP_ZOOM }
					/>
					...
					<TextControl
						...
						help={__('e.g., 100% or 600px', 'bawbab-interactive-maps')}
					/>
					<TextControl
					...
					<p style={{ fontSize: '11px', color: '#757575' }}>
                        {__('Note: Locations are managed in the ', 'bawbab-interactive-maps') }
                        <a href="admin.php?page=bawbab-interactive-maps-settings" target="_blank">
                            {__('Map Settings Page', 'bawbab-interactive-maps')}
                        </a>.
                    </p>
				</PanelBody>
			</InspectorControls>
