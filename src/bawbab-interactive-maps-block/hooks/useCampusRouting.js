...
export const useCampusRouting = () => {
	...
	const fetchGraph = useCallback( async () => {
		...
		try {
			...
			if ( ! response.ok )
				throw new Error(
					__(
						'Failed to load network graph maps.',
			...
			const normalizedEntries = ( data.entries || [] )
				.map( ( entry ) => {
					...
					return {
						...entry,
						unwrappedCoords: Array.isArray( coords )
							? [
									parseFloat( coords[ 0 ] ),
									parseFloat( coords[ 1 ] ),
							  ]
				...
				.filter(
					( e ) =>
						e.unwrappedCoords !== null &&
						! isNaN( e.unwrappedCoords[ 0 ] )
				);
			...
			data.network.forEach( ( segment ) => {
				...
				if (
					segment.geom.type === 'MultiLineString' &&
					Array.isArray( coords[ 0 ] )
				) {
					coords = coords[ 0 ];
				}
				...
				const startLat =
					typeof coords[ 0 ][ 1 ] === 'number'
						? coords[ 0 ][ 1 ]
						: parseFloat( coords[ 0 ][ 1 ] );
				const endLng =
					typeof coords[ coords.length - 1 ][ 0 ] === 'number'
						? coords[ coords.length - 1 ][ 0 ]
						: parseFloat( coords[ coords.length - 1 ][ 0 ] );
				const endLat =
					typeof coords[ coords.length - 1 ][ 1 ] === 'number'
						? coords[ coords.length - 1 ][ 1 ]
						: parseFloat( coords[ coords.length - 1 ][ 1 ] );
				...
				const startKey = `${ startLng.toFixed(
					6
				) },${ startLat.toFixed( 6 ) }`;
				const endKey = `${ endLng.toFixed( 6 ) },${ endLat.toFixed(
					6
				) }`;
			...
			console.error(
				'❌ [useCampusRouting] Graph Initialization Error:',
				err
			);
	...
	const findIntersectingDoors = useCallback( ( feature, loadedEntries ) => {
		...
		const matchedDoors = loadedEntries.filter( ( entry ) => {
			if ( entry.type !== 'door' || ! entry.unwrappedCoords )
				return false;
		...
		console.log(
			`🚪 [useCampusRouting] Restricted filter executed. Mapped ${ matchedDoors.length } doors.`
		);
		return matchedDoors;
	}, [] );
	...
	const calculateRoute = useCallback(
		async ( startEntry, endEntry ) => {
			...
			const startLng = startEntry.unwrappedCoords
				? startEntry.unwrappedCoords[ 0 ]
				: parseFloat( startEntry.lng );
			const startLat = startEntry.unwrappedCoords
				? startEntry.unwrappedCoords[ 1 ]
				: parseFloat( startEntry.lat );
			...
			if (
				! nearestNetworkNodeKey ||
				! adjList[ nearestNetworkNodeKey ]
			) {
				setError(
					__(
						'No structural network path lines could be aligned nearby.',
						'bawbab-interactive-maps'
					)
				);
				return null;
			...
			while ( queue.size > 0 ) {
				...
				queue.forEach( ( node ) => {
					if (
						shortestNode === null ||
						distances[ node ] < distances[ shortestNode ]
					) {
						shortestNode = node;
					}
				} );
				...
				if (
					shortestNode === endKey ||
					distances[ shortestNode ] === Infinity
				)
					break;
				...
				neighbors.forEach( ( neighbor ) => {
					...
					const alternatePathCost =
						distances[ shortestNode ] + neighbor.weight;
			...
			if ( distances[ endKey ] === Infinity ) {
				setError(
					__(
						'No routing connection possible between targets.',
						'bawbab-interactive-maps'
					)
				);
				return null;
			...
			const [ junctionLng, junctionLat ] = nearestNetworkNodeKey
				.split( ',' )
				.map( parseFloat );
			const projectionConnectorLine = [
				{
					lat: startLat,
					lng: startLng,
					floor: endEntry.floor,
					isProjectionConnectorArc: true,
				},
				{
					lat: junctionLat,
					lng: junctionLng,
