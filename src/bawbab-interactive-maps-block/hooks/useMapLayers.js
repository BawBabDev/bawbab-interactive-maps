...
export const useMapLayers = (spatialFeatures) => {
    const [visibleLayers, setVisibleLayers] = useState({
        markers: true, labels: true, parcels: true, buildings: true, paths: true, land_use: true
    });
    
    const [layerOpacity, setLayerOpacity] = useState({
        parcels: 1.0, buildings: 1.0, paths: 1.0, land_use: 1.0
    });

    const [activeFloor, setActiveFloor] = useState(0);

    const toggleLayer = (layer) => setVisibleLayers(prev => ({ ...prev, [layer]: !prev[layer] }));
    const handleOpacityChange = (layer, val) => setLayerOpacity(prev => ({ ...prev, [layer]: val }));
