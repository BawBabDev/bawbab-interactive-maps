...
const INNER_RADIUS_KM = 0.50; // Under 500m counts as on-site walker
...
export const useExternalRouting = () => {
    ...
    const [positionState, setPositionState] = useState({ onCampus: true, userCoords: null });
        ...
        return new Promise((resolve) => {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    const distance = computeHaversineDistance(uCoords, CAMPUS_CENTER);
                    
                    const isInsideProperty = distance <= INNER_RADIUS_KM;
                    const result = { onCampus: isInsideProperty, coords: uCoords };
                ...
                (error) => {
                    console.warn("Geolocation tracking denied. Defaulting to Internal Navigation:", error);
    ...
    const getHandoverLink = useCallback((entries, userCoords) => {
        ...
        const gates = entries.filter(e => e.type === 'main_road_entrance' || e.type === 'parking');
        const targetGate = gates.length > 0 ? gates[0] : { geom: { coordinates: [CAMPUS_CENTER.lng, CAMPUS_CENTER.lat] } };
        ...
        if (userCoords && gates.length > 0) {
            ...
            gates.forEach(gate => {
                const gCoords = { lat: gate.geom.coordinates[1], lng: gate.geom.coordinates[0] };
    ...
    return { checkUserProximity, getHandoverLink, isCalculating, positionState };
};