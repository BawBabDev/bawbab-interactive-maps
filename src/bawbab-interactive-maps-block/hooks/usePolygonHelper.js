...
export default function usePolygonHelper({ paths, options, onClick, onMouseEnter, onMouseLeave }) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    if (onClick) {
      listeners.push(
        google.maps.event.addListener(polygon, 'click', (e) => onClick(e))
      );
    }

    if (onMouseEnter) {
      listeners.push(
        google.maps.event.addListener(polygon, 'mouseover', (e) => onMouseEnter(e))
      );
    }

    if (onMouseLeave) {
      listeners.push(
        google.maps.event.addListener(polygon, 'mouseout', (e) => onMouseLeave(e))
      );
    }
